...
class ApplicationController @Inject()(cc: ControllerComponents) extends AbstractController(cc) {
  ...
  val MAXGUESTS = Integer.parseInt(ConfigFactory.load().getString("latches.MAXGUESTS"))
  ...
  def saveStock=Action{request =>
    ...
    if(json == null){
      ...
      try{
        ...
        state.addDataEntry(nachname, "^[A-z|üÜ|öÖ|äÄ|\\s|\\-\\.|0-9]{2,}")
        ...
        state.addDataEntry(vorname, "^[A-z|üÜ|öÖ|äÄ|\\s|\\-\\.|0-9]{2,}")
